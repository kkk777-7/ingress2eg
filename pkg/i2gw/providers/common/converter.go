...
func (a *ingressAggregator) toHTTPRoutesAndGateways(options i2gw.ProviderImplementationSpecificOptions) ([]httpRouteWithSources, []gwapiv1.Gateway, field.ErrorList) {
	var httpRoutes []httpRouteWithSources
	var errors field.ErrorList
	listenersByNamespacedGateway := map[string][]gwapiv1.Listener{}
	...
	for _, rgk := range ruleGroupsKeys {
		...
		if rg.host != "" {
			listener.Hostname = (*gwapiv1.Hostname)(&rg.host)
		} else if len(rg.tls) == 1 && len(rg.tls[0].Hosts) == 1 {
			listener.Hostname = (*gwapiv1.Hostname)(&rg.tls[0].Hosts[0])
		}
		if len(rg.tls) > 0 {
			listener.TLS = &gwapiv1.ListenerTLSConfig{}
		}
			...
			Spec: gwapiv1.HTTPRouteSpec{
				CommonRouteSpec: gwapiv1.CommonRouteSpec{
					ParentRefs: []gwapiv1.ParentReference{{
						Name: gwapiv1.ObjectName(db.ingressClass),
					}},
				},
			},
	...
	gatewaysByKey := map[string]*gwapiv1.Gateway{}
	for gwKey, listeners := range listenersByNamespacedGateway {
		...
		if gateway == nil {
			gateway = &gwapiv1.Gateway{
				...
				Spec: gwapiv1.GatewaySpec{
					GatewayClassName: gwapiv1.ObjectName(parts[1]),
				},
			}
		...
		for _, listener := range listeners {
			...
			gateway.Spec.Listeners = append(gateway.Spec.Listeners, gwapiv1.Listener{
				Name:     gwapiv1.SectionName(fmt.Sprintf("%shttp", listenerNamePrefix)),
			...
			if listener.TLS != nil {
				gateway.Spec.Listeners = append(gateway.Spec.Listeners, gwapiv1.Listener{
					Name:     gwapiv1.SectionName(fmt.Sprintf("%shttps", listenerNamePrefix)),
					Hostname: listener.Hostname,
					Port:     443,
...
func (rg *ingressRuleGroup) toHTTPRoute(servicePorts map[types.NamespacedName]map[string]int32, options i2gw.ProviderImplementationSpecificOptions) (gwapiv1.HTTPRoute, [][]providerir.BackendSource, field.ErrorList) {
	...
	if rg.ingressClass != "" {
		httpRoute.Spec.ParentRefs = []gwapiv1.ParentReference{{Name: gwapiv1.ObjectName(rg.ingressClass)}}
	}
	if rg.host != "" {
		httpRoute.Spec.Hostnames = []gwapiv1.Hostname{gwapiv1.Hostname(rg.host)}
	}
...
func (rg *ingressRuleGroup) configureBackendRef(servicePorts map[types.NamespacedName]map[string]int32, paths []ingressPath) ([]gwapiv1.HTTPBackendRef, []providerir.BackendSource, field.ErrorList) {
	var errors field.ErrorList
	...
	for i, path := range paths {
		...
		backendRefs = append(backendRefs, gwapiv1.HTTPBackendRef{BackendRef: *backendRef})
...
func toHTTPRouteMatch(routePath networkingv1.HTTPIngressPath, path *field.Path, toImplementationSpecificPathMatch i2gw.ImplementationSpecificHTTPPathTypeMatchConverter) (*gwapiv1.HTTPRouteMatch, *field.Error) {
	pmPrefix := gwapiv1.PathMatchPathPrefix
	pmExact := gwapiv1.PathMatchExact

	match := &gwapiv1.HTTPRouteMatch{Path: &gwapiv1.HTTPPathMatch{Value: &routePath.Path}}
